...
ПРАВИЛА ВХОДА:
- Система проверила score и confidence — они прошли минимальные пороги
- НО ты имеешь право SKIP если видишь ЯВНОЕ противопоказание в данных:
  • RSI экстремум на H4 ПРОТИВ направления (H4 RSI < 35 для short, > 65 для long)
  • Цена у 24h low/high ПРОТИВ направления (шорт у дна, лонг у вершины)
  • H4 тренд ПРОТИВОРЕЧИТ сигналу + слабый momentum
  • Orderbook imbalance ПРОТИВ направления (> 1.5)
- ENTER если: направление подтверждено на нескольких ТФ + нет явных противопоказаний выше
- НЕ отказывай по субъективным причинам ("мне кажется рынок развернётся")

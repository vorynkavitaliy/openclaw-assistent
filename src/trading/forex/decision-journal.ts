...
const log = createLogger('forex-decision-journal');

const JOURNAL_FILE = config.decisionsFile;
...
export interface Decision {
  ...
  data: {
    ...
    atr?: number;
    orderId?: string;
    llmDecision?: string; // ENTER | SKIP | WAIT
    llmReason?: string;
  };
  outcome?: {
    pnl?: number;
...
export function formatDecision(d: Decision): string {
  ...
  if (d.data.confluenceScore !== undefined) {
    ...
    if (d.data.bias !== undefined) lines.push(`Bias: ${d.data.bias}`);
    lines.push('');
  }
  ...
  if (d.data.llmDecision !== undefined) {
    lines.push('LLM РЕШЕНИЕ:');
    lines.push(`  Решение: ${d.data.llmDecision}`);
    if (d.data.llmReason) lines.push(`  Причина: ${d.data.llmReason}`);
    lines.push('');
  }
  ...
  if (d.data.entry !== undefined) {
    ...
    if (d.data.lots !== undefined) lines.push(`  Lots: ${d.data.lots}`);
    if (d.data.slPips !== undefined) lines.push(`  SL pips: ${d.data.slPips}`);
    if (d.data.tpPips !== undefined) lines.push(`  TP pips: ${d.data.tpPips}`);
    if (d.data.rr !== undefined) lines.push(`  R:R: ${d.data.rr}`);
    if (d.data.atr !== undefined) lines.push(`  ATR: ${d.data.atr}`);
    lines.push('');
  }
...
export function formatSummary(summary: DecisionSummary): string {
  const lines: string[] = [];
  lines.push(`=== Дневник форекс-решений (${summary.period}) ===`);

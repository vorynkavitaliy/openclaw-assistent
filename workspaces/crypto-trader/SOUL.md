...
- Call 1: check script → raw data (balance, positions, H4+M15 indicators, funding, OI, sentiment, tasks)
...
1. **ALWAYS have at least 1 limit order or position** — 0 orders + 0 positions = FORBIDDEN
...
7. Exception: daily loss limit hit or kill-switch ON
...
6. **Daily loss limit**: if 4% lost in a day — stop trading (HyroTrade buffer 5%)
7. **No impulses**: entry only on clear strategy signal
8. **No trading on news**: 30 min before and after FOMC/CPI — don't trade
...
12. **Leverage**: MAXIMUM 5x, default 3x. NEVER more.
13. **Funding rate**: at extreme funding (>0.05% or <-0.05%) — don't enter against trend
14. **Liquidation**: if margin ratio > 80% — immediately reduce position
15. **Low-cap coins**: market cap > $100M, allocation ≤ 5% of balance
